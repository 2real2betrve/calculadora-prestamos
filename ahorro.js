// ===================================
// CALCULADORA DE AHORRO - ahorro.js (ESPAÑOL)
...
function calculateSavings() {
    ...
    if (isNaN(initialAmount) || initialAmount < 0) {
        alert('Por favor, ingresa un capital inicial válido');
        return;
    }
    ...
    if (savingsGoal > 0) {
        if (goalReached) {
            ...
            alertBox.innerHTML = `
                <strong>¡Meta alcanzada!</strong> Lograrás tu meta de ${formatCurrency(savingsGoal)} 
                en <strong>${yearsToGoal} años${monthsToGoal > 0 ? ' y ' + monthsToGoal + ' meses' : ''}</strong>.
            `;
        } else {
            alertBox.className = 'alert-box alert-warning';
            alertBox.innerHTML = `
                <strong>Meta no alcanzada:</strong> Al final de ${years} años tendrás ${formatCurrency(balance)}, 
                lo cual es <strong>${formatCurrency(savingsGoal - balance)}</strong> menos que tu meta. 
                Considera aumentar tus aportaciones o el período de ahorro.
            `;
        }
    } else {
        alertBox.innerHTML = '';
...
function generateChart(monthlyData, totalContributed) {
    ...
    monthlyData.forEach((data, index) => {
        if (index % 12 === 0 || index === monthlyData.length - 1) {
            labels.push(`Año ${Math.ceil(data.month / 12)}`);
